...
d_gh_ch_fante_23 <- read.csv("/Users/kweisman/Desktop/TEMP_GHANA_2018/beetles_ghana_fante_children_tidy_2018-07-19.csv")[-1] %>% 
  rename(subnum = subid) %>% 
...
Minimizing BIC suggests retaining `r d_gh_ch_fante_23_bic$vss.stats %>% rownames_to_column("nfact") %>% filter(BIC == min(BIC)) %>% select(nfact) %>% as.numeric()` factors: See 3-factor solution, above.
...
fa(d_gh_ch_fante_23, 4, "oblimin")$scores[] %>%
  ...
  # mutate(subid = paste("ghana", subid, sep = "_")) %>%
  gather(factor, score, -subid) %>%
  full_join(read.csv("/Users/kweisman/Desktop/TEMP_GHANA_2018/beetles_ghana_fante_children_tidy_2018-07-19.csv") %>% distinct(subid, character)) %>%
  group_by(character, factor) %>%
  do(data.frame(rbind(smean.cl.boot(.$score)))) %>%
  ungroup() %>%
  mutate(character = factor(character,
...
d_gh_ch_fante_22 <- read.csv("/Users/kweisman/Desktop/TEMP_GHANA_2018/beetles_ghana_fante_children_tidy_2018-07-19.csv") %>% 
  rename(subnum = subid) %>% 
  filter(grepl("fante", tolower(language_home)) | grepl("twi", tolower(language_home))) %>%
  clean_fun(key = question_key, ex_addsub = TRUE)
...
Parallel analysis suggests retaining `r d_gh_ch_fante_22_parallel$nfact` factors: See 3-factor solution, above.

```{r, include = F}
d_gh_ch_fante_22_bic <- VSS(d_gh_ch_fante_22, n = max_fact_fun(ncol(d_gh_ch_fante_22)), rotate = "oblimin", plot = F)
```

Minimizing BIC suggests retaining `r d_gh_ch_fante_22_bic$vss.stats %>% rownames_to_column("nfact") %>% filter(BIC == min(BIC)) %>% select(nfact) %>% as.numeric()` factors: 
...
subids <- read.csv("/Users/kweisman/Documents/Research (Stanford)/Projects/Templeton Grant/DEVELOPMENTAL TASKS/beetles:dimkid:factors/analysis/_US pilot/beetles_pilot2_tidy.csv") %>% distinct(subid, character) %>%
  ...
  full_join(read.csv("/Users/kweisman/Desktop/TEMP_GHANA_2018/beetles_ghana_fante_children_tidy_2018-07-19.csv") %>% 
              distinct(subid, character) %>%
